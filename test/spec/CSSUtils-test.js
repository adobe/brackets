...
define(function (require, exports, module) {
    ...
    describe("CSS Utils", function () {
        ...
        it("should find the first instance of the h2 selector", function () {
            var selector = CSSUtils.findSelector(content, "h2");
            
            expect(selector).not.toBe(null);
            expect(selector.start).toBe(4367);
            expect(selector.end).toBe(4483);
        });
        
        it("should find all instances of the h2 selector", function () {
            var selectors = CSSUtils.findAllMatchingSelectors(content, "h2");
            expect(selectors).not.toBe(null);
            expect(selectors.length).toBe(2);
            expect(selectors[0].start).toBe(4367);
            expect(selectors[0].end).toBe(4483);
            expect(selectors[1].start).toBe(4665);
            expect(selectors[1].end).toBe(4713);
        });
        
        it("should return null when a findSelector cannot find a match", function () {
            var selector = CSSUtils.findSelector(content, "NO-SUCH-SELECTOR");
            expect(selector).toBe(null);
        });
        
        it("should return an empty array when findAllMatchingSelectors() can't find any matches", function () {
            var selectors = CSSUtils.findAllMatchingSelectors(content, "NO-SUCH-SELECTOR");
