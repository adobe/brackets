...
CAsyncDirWatchInfo::sEntry::sEntry(LPWSTR lpszFilename, int iFilenameLen, DWORD dwAction) :
m_dwAction(dwAction),
m_pPrevious(NULL),
m_pNext(NULL)
{
	m_lpszFilename = (LPWSTR)malloc(sizeof(WCHAR) * (iFilenameLen + 1));
	if (m_lpszFilename != NULL)
	{
		memcpy(m_lpszFilename, lpszFilename, sizeof(WCHAR) * iFilenameLen);
		m_lpszFilename[iFilenameLen] = 0x00;	// null terminate
	}
}
...
void NodeFSEvents::Startup(const char *lpszPath)
{
	...
	{
		::MultiByteToWideChar(CP_UTF8, 0, lpszPath, -1, m_szPathName, MAXPATH+1);
...
void NodeFSEvents::Callback(uv_async_t *handle, int status)
{
	...
	{
		int ilen = ::WideCharToMultiByte(CP_UTF8, 0, entry->m_lpszFilename, -1, NULL, 0, NULL, NULL);
		LPSTR lpsz = (LPSTR)malloc(sizeof(char) * (ilen + 1));
		if (lpsz != NULL)
		{
			::WideCharToMultiByte(CP_UTF8, 0, entry->m_lpszFilename, -1, lpsz, ilen + 1, NULL, NULL);

			// call back into JS with each change.
			args[1] = String::New(lpsz);
			args[2] = Integer::New(entry->m_dwAction);
