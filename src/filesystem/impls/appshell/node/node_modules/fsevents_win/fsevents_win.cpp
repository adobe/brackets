...
CAsyncDirWatchInfo::sEntry::sEntry(LPWSTR lpszFilename, int iFilenameLen) :
m_previous(NULL),
m_next(NULL)
{
...
void CAsyncDirWatchInfo::Enqueue(LPENTRY lpEntry)
{
	...
	if (lpEntry != NULL)
	{
		...
		{
			// first one in list
			m_tail = m_head = lpEntry;
		}
		else
		{
			// append to list
			m_tail->m_next = lpEntry;
			lpEntry->m_previous = m_tail;
			m_tail = lpEntry;
		}
	}
	if (m_hMutex != NULL)
...
CAsyncDirWatchInfo::LPENTRY CAsyncDirWatchInfo::Dequeue()
{
	...
	if (m_head != NULL)
	{
		...
		lpEntry->m_next = lpEntry->m_previous = NULL;
	}
	if (m_hMutex != NULL)
		::ReleaseMutex(m_hMutex);
	return lpEntry;
...
void NodeFSEvents::Callback(uv_async_t *handle, int status)
{
	...
	CAsyncDirWatchInfo::LPENTRY entry = This->m_pWatchInfo->Dequeue();
	if (entry != NULL)
	{
		delete entry;
	}
/*
	...
	while (item) {
		...
		args[1] = String::New(item->path ? item->path : "");
		args[2] = Integer::New(item->flags);
		args[3] = Integer::New(item->evtid);
