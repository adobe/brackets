//
// MORE - Leaner CSS v1.3.1
// Simply a renamed version of LESS
// http://lesscss.org
// 
...
function require(arg) {
    return window.more[arg.split('/')[1]];
};
...
if (typeof environment === "object" && ({}).toString.call(environment) === "[object Environment]") {
    ...
    if (typeof(window) === 'undefined') { more = {} }
    else                                { more = window.more = {} }
    tree = more.tree = {};
    more.mode = 'rhino';
    ...
    // Browser
    if (typeof(window.more) === 'undefined') { window.more = {} }
    more = window.more,
    tree = window.more.tree = {};
    more.mode = 'browser';
}
//
// more.js - parser
...
//
//
more.Parser = function Parser(env) {
    var input,       // MoRE input string
    ...
    var imports = this.imports = {
        ...
        mime:  env && env.mime,         // MIME type of .more files
        ...
        push: function (path, callback) {
            ...
            //
            more.Parser.importer(path, this.paths, function (e, root) {
    ...
    function $(tok) {
        ...
        // which come after that. The reason for this is that MoRE's
    ...
    function getFileName(e) {
        if(more.mode === 'browser' || more.mode === 'rhino')
            return e.filename;
        else
    ...
    // the lower the number, the more nodes it will create in the tree.
    ...
    return parser = {
        ...
        parse: function (str, callback) {
            ...
            try {
                ...
                return callback(new(MoreError)(e, env));
            }
            ...
            root.toCSS = (function (evaluate) {
                ...
                return function (options, variables) {
                    ...
                    try {
                        ...
                        throw new(MoreError)(e, env);
                    }

                    if ((importError = parser.imports.error)) { // Check if there was an error during importing
                        if (importError instanceof MoreError) throw importError;
                        else                                  throw new(MoreError)(importError, env);
                    }

                    if (options.yuicompress && more.mode === 'node') {
        ...
        parsers: {
            ...
            // but keep the MoRE comments `//` silent, by just skipping
            // over them.
            comment: function () {
                var comment;
...
if (more.mode === 'browser' || more.mode === 'rhino') {
    ...
    //
    more.Parser.importer = function (path, paths, callback, env) {
...
tree.Import = function (path, imports, features, once, index) {
    ...
    // The '.more' extension is optional
    if (path instanceof tree.Quoted) {
        this.path = /\.(le?|c)ss(\?.*)?$/.test(path.value) ? path.value : path.value + '.more';
    } else {
...
more.env = more.env || (location.hostname == '127.0.0.1' ||
...
//
more.async = more.async || false;
more.fileAsync = more.fileAsync || false;

// Interval between watch polls
more.poll = more.poll || (isFileProtocol ? 1000 : 1500);

//
// Watch mode
//
more.watch   = function () { return this.watchMode = true };
more.unwatch = function () { return this.watchMode = false };

if (more.env === 'development') {
    more.optimization = 0;
    ...
    if (dumpLineNumbers) {
        more.dumpLineNumbers = dumpLineNumbers[1];
    }
    more.watchTimer = setInterval(function () {
        if (more.watchMode) {
...
//
// Get all <link> tags with the 'rel' attribute set to "stylesheet/more"
//
var links = document.getElementsByTagName('link');
var typePattern = /^text\/(x-)?more$/;

more.sheets = [];

for (var i = 0; i < links.length; i++) {
    if (links[i].rel === 'stylesheet/more' || (links[i].rel.match(/stylesheet/) &&
       (links[i].type.match(typePattern)))) {
        more.sheets.push(links[i]);
    }
}


more.refresh = function (reload) {
...
more.refreshStyles = loadStyles;

more.refresh(more.env === 'development');
    ...
    for (var i = 0; i < styles.length; i++) {
        if (styles[i].type.match(typePattern)) {
            new(more.Parser)({
                ...
                dumpLineNumbers: more.dumpLineNumbers
...
function loadStyleSheets(callback, reload) {
    for (var i = 0; i < more.sheets.length; i++) {
        loadStyleSheet(more.sheets[i], callback, reload, more.sheets.length - (i + 1));
    }
}
            ...
            try {
                ...
                new(more.Parser)({
                    ...
                    try {
                        ...
                        removeNode(document.getElementById('more-error-message:' + extractId(href)));
                    } catch (e) {
                        error(e, href);
                    }
                });
...
function createCSS(styles, sheet, lastModified) {
    ...
    var id = 'more:' + (sheet.title || extractId(href));
...
function xhr(url, type, callback, errback) {
    ...
    var async = isFileProtocol ? more.fileAsync : more.async;

    if (typeof(xhr.overrideMimeType) === 'function') {
        xhr.overrideMimeType('text/css');
    }
    xhr.open('GET', url, async);
    xhr.setRequestHeader('Accept', type || 'text/x-more, text/css; q=0.9, */*; q=0.5');
    xhr.send(null);

    if (isFileProtocol && !more.fileAsync) {
...
function log(str) {
    if (more.env == 'development' && typeof(console) !== "undefined") { console.log('more: ' + str) }
}

function error(e, href) {
    var id = 'more-error-message:' + extractId(href);
    ...
    elem.id        = id;
    elem.className = "more-error-message";

    content = '<h3>'  + (e.message || 'There is an error in your .more file') +
    ...
    createCSS([
        '.more-error-message ul, .more-error-message li {',
            'list-style-type: none;',
            'margin-right: 15px;',
            'padding: 4px 0;',
            'margin: 0;',
        '}',
        '.more-error-message label {',
            'font-size: 12px;',
            'margin-right: 15px;',
            'padding: 4px 0;',
            'color: #cc7777;',
        '}',
        '.more-error-message pre {',
            'color: #dd6666;',
            'padding: 4px 0;',
            'margin: 0;',
            'display: inline-block;',
        '}',
        '.more-error-message pre.line {',
            'color: #ff0000;',
        '}',
        '.more-error-message h3 {',
            'font-size: 20px;',
            'font-weight: bold;',
            'padding: 15px 0 5px 0;',
            'margin: 0;',
        '}',
        '.more-error-message a {',
            'color: #10a',
        '}',
        '.more-error-message .error {',
            'color: red;',
            'font-weight: bold;',
            'padding-bottom: 2px;',
...
// amd.js
//
// Define More as an AMD module.
if (typeof define === "function" && define.amd) {
    define("more", [], function () { return more; } );
}
})(window);
